...
class EnWideScreenVideo {
    ...
    constructor(section) {
        ...
        const supportsDash = window.MediaSource && MediaSource.isTypeSupported(`video/mp4; codecs="avc1.4D401E, mp4a.40.2"`);
        const canPlayHls = document.createElement('video').canPlayType('application/vnd.apple.mpegurl');

        if (supportsDash || canPlayHls) {
            fetch(this.source, {
                method: 'HEAD',
                headers: {
                    accept: supportsDash ? 'application/dash+xml' : 'application/vnd.apple.mpegurl'
                }
            })
                .then(function (response) {
                    ...
                    } else if (contentType && contentType.indexOf('mpegurl') != -1) {
